...
## install\_url and install\_resource

If `install_url` is set, then the file it points to is downloaded to the local
cache (using `remote_file`) and is used as the `source` for the package install.

As the `package` resource is not always able to work with `source` (for example
`package` on ubuntu does not allow this) you can also set `install_resource` to
a specific package resource (for example ubuntu would need `:dpkg_package`).
If `install_resource` is left to `nil` the standard `package` resource is used.
...
## template\_type\_string

As of version 0.14 of fluentd the `type` configuration parameter was replaced
with `@type` but the cookbook still hardcodes the old format.

You can set the `template_type_string` cookbook parameter to `@type` to use
the new parameter format everywhere in templates generated by this cookbook.
